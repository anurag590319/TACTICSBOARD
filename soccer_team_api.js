...
// Adds dragging functionality to a given element
function addDraggingFunctionality(element, container) {
    ...
    document.addEventListener('mousemove', (e) => {
        if (isDragging) {
            ...
            let x = e.clientX - containerRect.left - element.clientWidth / 2;
            let y = e.clientY - containerRect.top - element.clientHeight / 2;

            x = Math.max(0, Math.min(x, container.clientWidth - element.clientWidth));
            y = Math.max(0, Math.min(y, container.clientHeight - element.clientHeight));
